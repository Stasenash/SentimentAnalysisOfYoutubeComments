...
class AnalysisOfOneChannelActions:
    ...
    def __init__(self, channel_link):
        ...
        self.stopwords = ['это', 'еще', 'ещё', 'и', 'в', 'во', 'не', 'что', 'он', 'на', 'я', 'с', 'со', 'как', 'а',
                          'то', 'все', 'она', 'так', 'его', 'но', 'да', 'ты', 'к', 'у', 'же', 'вы', 'за', 'бы', 'по',
                          'только', 'ее', 'мне', 'было', 'вот', 'от', 'меня', 'еще', 'нет', 'о', 'из', 'ему', 'теперь',
                          'когда', 'даже', 'ну', 'вдруг', 'ли', 'если', 'уже', 'или', 'ни', 'быть', 'был', 'него', 'до',
                          'вас', 'нибудь', 'опять', 'уж', 'вам', 'ведь', 'там', 'потом', 'себя', 'ничего', 'ей',
                          'может', 'они', 'тут', 'где', 'есть', 'надо', 'ней', 'для', 'мы', 'тебя', 'их', 'чем', 'была',
                          'сам', 'чтоб', 'без', 'будто', 'чего', 'раз', 'тоже', 'себе', 'под', 'будет', 'ж', 'тогда',
                          'кто', 'этот', 'того', 'потому', 'этого', 'какой', 'совсем', 'ним', 'здесь', 'этом', 'один',
    ...
    def data_full_check(self, dictionary):
        ...
        try:
            relation = str(
                f"""лайков - {round(int(dictionary['statistics']['likeCount']) / (int(dictionary['statistics']['likeCount']) + int(dictionary['statistics']['dislikeCount'])) * 100)}%, дизлайков - {round(int(dictionary['statistics']['dislikeCount']) / (int(dictionary['statistics']['likeCount']) + int(dictionary['statistics']['dislikeCount'])) * 100)}%""")
        except:
        ...
        return [video_id, channel_name, video_name, commentCount, likesCount, dislikesCount, relation, viewsCount,
                tagsCount, video_duration, video_description]
...
{info['snippet']['description']}
    ...
    def alysis_histogram(self, massive_pos_1, massive_neg_1, massive_pos_2, massive_neg_2, massive_pos_3,
                         massive_neg_3):
    ...
    def get_info_from_video(self):
        ...
        for _id in ids:
            ...
            try:
                ...
                self.massive_pos_1.append(float(string[1][1:4]))
                self.massive_neg_1.append(float(string[2][1:4]))
                self.massive_pos_2.append(float(string[3][1:4]))
                self.massive_neg_2.append(float(string[4][1:4]))
                self.massive_pos_3.append(float(string[5][1:4]))
                self.massive_neg_3.append(float(string[6][1:4]))
            except:
                pass
    ...
    def analysis_of_comments_for_a_single_channel(self):
        ...
        info += f"Максимальное количество комментариев: {max(self.commentsCount)}" + "\n"
        info += f"Минимальное количество комментариев: {min(self.commentsCount)}" + "\n"
        info += f"Максимальное количество лайков: {max(self.likes)}" + "\n"
        info += f"Минимальное количество лайков: {min(self.likes)}" + "\n"
        info += f"Максимальное количество дизлайков: {max(self.dislikes)}" + "\n"
        info += f"Минимальное количество дизлайков: {min(self.dislikes)}" + "\n"
        info += f"Максимальное количество просмотров: {max(self.views)}" + "\n"
        info += f"Минимальное количество просмотров: {min(self.views)}" + "\n"
